...
@section('content')
	<div class="add-franchise-super-main">
		...
		<p><a href="#">Client </a> / </a>Edit Client Information</p>
			    ...
			    <div class="view-tab-content-main">
			    	...
			    	<div class="view-tab-content-head-main view-tab-content-head-main-3">
			    		...
			    		<div class="view-tab-content-butn">
			    			<button type="button" class="btn add-franchise-data-butn-1" id="save-client-information"><i class="fa fa-check" aria-hidden="true"></i>Save Changes</button>
			    		</div>
			    	...
			    	<div class="super-admin-add-relation-main spinner_main">
			    		...
			    		<div class="super-admin-add-relation border-bot-0">
                            ...
                            <figure>	
                                ...
                                <label class="txt-bold radio-inline w-auto"><input name="chooselocation_interest" @if($Client->chooselocation_interest == 'Office') checked="" @endif class="pad-r w-auto" type="radio" value="Office">Office</label>
                                <label>&nbsp;</label>
                                ...
                                <label>&nbsp;</label>
								<label class="txt-bold radio-inline w-50"><input name="chooselocation_interest" @if($Client->chooselocation_interest == 'School') checked="" @endif class="pad-r w-auto" type="radio" value="School">School</label>
                            ...
                            <figure>	
								...
								<!--<label class="txt-bold radio-inline"><input name="client_crew" @if($Client->client_crew == 'Susan Fletcher') checked="" @endif class="pad-r w-auto" type="radio" value="Susan Fletcher">Susan Fletcher</label>
                                <label>&nbsp;</label>
								<label class="txt-bold radio-inline"><input name="client_crew" @if($Client->client_crew == 'Tom Hardy') checked="" @endif class="pad-r w-auto" type="radio" value="Tom Hardy">Tom Hardy</label>
                                <label>&nbsp;</label>
								<label class="txt-bold radio-inline"><input name="client_crew" @if($Client->client_crew == 'Methew Perry') checked="" @endif class="pad-r w-auto" type="radio" value="Methew Perry">Methew Perry</label>
                                <br/>-->
                                <?php /*?><select name="client_crew">
                                    <option value="">Select Crew</option>
                                    @if($femployees)
                                        @foreach($femployees as $femployee)
                                            <option value="{{ $femployee->personal_name }}" @if($Client->client_crew == $femployee->personal_name) selected="" @endif>{{ $femployee->personal_name }}</option>
                                        @endforeach
                                    @endif
                                </select><?php */?>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		///////////////////////////////////////////////
		$('#save-client-information').click(function () {
			var $valid = $("#editClient").valid();
			var client_email = $('input[name=client_email]').val();
			var client_id = '{{ $Client->id }}';
			if(client_email != '' && $valid){
				$.ajax({
					url:'{{ url("parent/client/emailexist") }}',
					type:'POST',
					dataType:'json',
