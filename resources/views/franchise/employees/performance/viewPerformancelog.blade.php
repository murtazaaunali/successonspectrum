...
@section('content')
	...
	<div class="add-franchise-super-main">
		<div class="view-tab-control">
			...
			<p>Employee / {{ $Employee->fullname }} / <span id="change-bread-crumb">Performance Log</span></p>-->
            <h6>{{ $Employee->personal_name }}</h6>
			<p>Employee / {{ $Employee->personal_name }} / <span id="change-bread-crumb">Performance Log</span></p>
		</div>
		...
		<div class="add-franchise-data-main-1 PerformanceNone">
			<ul class="nav nav-tabs">
			  ...
			  <li class="trigger"><a href="{{ url('franchise/employee/viewtimepunches/'.$Employee->id) }}" >Time Punches</a></li>
				    ...
				    <div class="upcoming-contact-expiration upcoming-contact-expiration-view-table padd-upcoming EmployeePerformanceMainDiv">
				    	...
				    	<div class="view-tab-content-head-main view-tab-content-head-main-padd">
				    		<div class="view-tab-content-head">
                                ...
                                <h3>{{ $Employee->personal_name }}'s Performance Log</h3>
				    		</div>
				    		<div class="view-tab-content-butn">
				    			@if(Request::has('performance_log'))
				    				...
				    				@else
				    					<a href="{{ url('franchise/employee/viewperformancelog/'.$Employee->id) }}" class="btn add-franchise-data-butn-1"><i class="fa fa-chevron-left" aria-hidden="true"></i> Back</a>
				    				@endif
				    			@else
				    				@if(!$Employee_performance->isEmpty())
				    					<a href="{{ url('franchise/employee/viewperformancelog/'.$Employee->id.'?performance_log=edit') }}" class="btn add-franchise-data-butn-1"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Edit Details</a>
				    				@endif
				    			@endif
				    			<a href="{{ url('franchise/employee/addperformance/'.$Employee->id) }}" class="btn  add-franchise-data-butn-1"><i class="fa fa-plus" aria-hidden="true"></i>Add Performance Log</a>
				    		</div>
				    		<div class="clearfix"></div>
				    	</div>
						<div class="super-admin-table-select padd-upcoming-1">
							<form action="" method="get" id="performancelog_filter">
								...
								<div class="ButtonsPullRightRisponsive">
									...
									<a href="{{ url('franchise/employee/viewperformancelog/'.$Employee->id) }}@if(Request::has('performance_log')){{ '?performance_log=edit' }}@endif " class="btn francse-filter-butn butn-spacing-padd-1"><i class="fa fa-filter" aria-hidden="true"></i>Clear Filter</a>
								</div>
							</form>
						</div>
						<div class="EmployeePerformanceTableMain">
							<div class="PerformanceLogTableWidth Edit-Performance super-admin-table-1 table-responsive super-admin-table-time-punches">
								<form action="{{ url('franchise/employee/performanceupdate/'.$Employee->id) }}" autocomplete="off" method="post" id="editPerformance">
										...
										@if(!$Employee_performance->isEmpty())
											...
											@foreach($Employee_performance as $log)
												<tr>
													...
													<td>
														@if(Request::has('performance_log'))
															<?php /*?><input type="hidden" name="_token" value="{{ csrf_token() }}"/><?php */?>
													...
													<td class="super-admin-table-time-punches-td">
														@if(Request::has('performance_log') == 'edit')
															...
															<a class="BluePencile"><i class="fa fa-pencil" aria-hidden="true"></i></a>
															<a href="#" class="red-clr delete_performance" data-employee_id="{{ $log->admin_employee_id }}" data-log_id="{{ $log->id }}"><i class="fa fa-trash" aria-hidden="true"></i></a>
														@else
															{{ $log->description }}
														@endif
													</td>
...
<script type="text/javascript">
	$(document).ready(function(e) {
		...
		$('.popup-delete-butn').click(function(){
			...
			$('#myModal2').modal('hide');*/
			window.location.href = '{{ url("franchise/employee/performancedelete") }}'+'/'+employee_id+'/'+log_id;
		});
