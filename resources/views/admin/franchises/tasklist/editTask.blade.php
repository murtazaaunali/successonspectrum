...
@section('content')
   <div class="add-employee-tabs-main-main add-employee-tabs-main-main add-franchise-super-main FranchiseEditTastListMain"> 
    ...
    <form action="{{ url('admin/franchise/edittasklist/'.$Franchise->id) }}" method="post" id="newTask">
			    	...
			    	<div class="employee-content-left add-franchise-lines-data">
				      	<div class="edit-task-list-li-edit">
						    <ul class="sortableList">
						        ...
						        @php 
						        	$taskList = $Franchise->tasklist()->orderBy('sort','asc')->get();
						        	$sort = 0;
						        @endphp 
						        	@foreach($taskList as $task)
								        <li class="ui-state-default" data-sort="{{ $sort }}"><label>:::</label><p class="task_headline">{{ $task->task }}</p>
								        	...
								        	<input type="hidden" name="task[{{$sort}}][franchise_id]" value="{{ $Franchise->id }}"/>
...
<div class="modal fade" id="myAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ url('admin/franchise/addtasklist/'.$Franchise->id) }}" method="post" id="addNewTask">
			<input type="hidden" name="_token" value="{{ csrf_token() }}"/>
  ...
  <div class="modal fade" id="myModaldel" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content DeleteEmployeepopup">
        ...
        <div class="modal-body">
          <p>Are you sure you want to delete this task permanently ? This action cannot be undone</p>
          <input class="btn popup-delete-butn" type="button" value="Delete Task">
        </div>
      </div>
	...
	$(document).ready(function(){
	    $(".sortableList").sortable({
		    ...
		    update: function(e,ui) {
				...
				$.ajax({
					url:'{{ url("admin/franchise/edittasklist") }}/{{ $Franchise->id }}',
					type:'POST',
					dataType:'json',
					data:formData,
		...
		$('.popup-delete-butn').click(function(){
			var Url = '{{ url("admin/franchise/deletetask/") }}';
			var Franchise_id = '{{ $Franchise->id }}';
			window.location.href = Url+'/'+Franchise_id+'/'+task_del;
		});
	});	
