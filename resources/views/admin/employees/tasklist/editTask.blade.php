   ...
   <div class="add-employee-tabs-main-main add-employee-tabs-main-main add-franchise-super-main"> 
    <div class="main-head-franchise">
        <div class="main-deck-head main-deck-head-franchise view-tab-control">
			...
			<p><a href="{{ url('admin/employees') }}">Employee</a> / <a href="{{ url('admin/employee/view/') }}/{{ $Employee->id }}">{{ $Employee->fullname }} </a> / Task List</p>
        </div>
    ...
    <form action="{{ url('admin/employee/edittasklist/'.$Employee->id) }}" method="post" id="newTask">
		...
		<div class="view-tabs-control-main">
			<div class="view-tab-control">
				<ul class="nav nav-tabs">
				  ...
				  <a href="{{ url('admin/employee/view/'.$Employee->id) }}">Employee Demographic</a></li>
				  ...
				  <li class="trigger task-btn"><a href="{{ url('admin/employee/viewtimepunches/'.$Employee->id) }}">Time Punches</a></li>-->
				  <li class="trigger-1"><a href="{{ url('admin/employee/viewperformancelog/'.$Employee->id) }}">Performance Log</a></li>
				</ul>
			</div>
				...
				<div class="add-franchise-data-main-1 upcoming-contact-expiration-view-table padd-zero-lines">
			    	<div class="view-tab-content-head-main">
			    		...
			    		<div class="view-tab-content-butn">
			    			<a href="javascript:void(0);" class="btn add-franchise-data-butn-1 task_add"><i class="fa fa-plus" aria-hidden="true"></i>Add To Task List</a>
			    	...
			    	<div class="employee-content-left add-franchise-lines-data ">
				      	<div class="edit-task-list-li-edit">
						    <ul class="sortableList">
						        ...
						        @php 
						        	$taskList = $Employee->tasklist()->orderBy('sort','asc')->get();
						        	$sort = 0;
						        @endphp 
						        	@foreach($taskList as $task)
								        <li class="ui-state-default" data-sort="{{ $sort }}"><label>:::</label><p class="task_headline">{{ $task->task }}</p>
								        	<span>
								        		...
								        		@else
								        			<button type="button" class="btn butn-edit-lines-edit change_status"><i class="fa fa-check" aria-hidden="true"></i>Mark Complete</button>
								        		@endif
								        	</span>	
								        	<div class="clearfix"></div>
								        	<input type="hidden" class="task_sort" name="task[{{$sort}}][sort]" value="{{ $sort }}"/>
								        	<input type="hidden" name="task[{{$sort}}][task_id]" value="{{ $task->id }}"/>
								        	<input type="hidden" name="task[{{$sort}}][employee_id]" value="{{ $Employee->id }}"/>
...
<div class="modal fade" id="myAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ url('admin/employee/addtasklist/'.$Employee->id) }}" method="post" id="addNewTask">
			<input type="hidden" name="_token" value="{{ csrf_token() }}"/>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary add_task">Add To Task List</button>
      </div>
    </div>
  </div>
</div>
	...
	$(document).ready(function(){
	    $(".sortableList").sortable({
		    ...
		    update: function(e,ui) {
				...
				$.ajax({
					url:'{{ url("admin/employee/edittasklist") }}/{{ $Employee->id }}',
					type:'POST',
					dataType:'json',
					data:formData,
	    ...
	    $('.change_status').click(function(){
	    	if($(this).hasClass('red-clr-butn-edit')){
				...
				$(this).html('<i class="fa fa-check" aria-hidden="true"></i>Mark Complete');
		...
		$('.popup-delete-butn').click(function(){
			var Url = '{{ url("admin/employee/deletetask/") }}';
			var Employee_id = '{{ $Employee->id }}';
			window.location.href = Url+'/'+Employee_id+'/'+task_del;
		});
	});	
</script>
