...
@section('content')
						...
						<div class="super-admin-table-select padd-upcoming-1">
							<form action="" method="get" id="timepunch_filter">
								...
								<div class="ButtonsPullRightRisponsive">
									...
									<a href="{{ url('admin/employee/viewtimepunches/'.$Employee->id) }}" class="btn francse-filter-butn butn-spacing-padd-1"><i class="fa fa-filter" aria-hidden="true"></i>Clear Filter</a>
								</div>
							</form>
						</div>
						<div class="super-admin-table-1 padd-upcoming-2">
							<div class="table-responsive">
								<table class="table-striped TimePuncheTableWidth">
                                    ...
                                    $Employee_title = Auth::guard('admin')->user()->employee_title; 
									@endphp
                                    <tr>
										<th>Date</th>
										<th>Days</th>
										...
										<th>PTO</th>
										<th>Remarks</th>
                                        @if($Employee_title == 'Director of Administration')<th>Status</th>@endif
									</tr>

									@if(!is_array($Employee_timepunches) && !$Employee_timepunches->isEmpty())
										
										@foreach($Employee_timepunches as $key=>$timepunch)
										...
										@endphp
											<tr>
												<?php /*?><td>{{ date('jS M Y', strtotime($timepunch->date)) }}</td><?php */?>
                                                ...
                                                @if($Employee_title == 'Director of Administration')
                                                <td>
                                                    @if($timepunch->status == 'Pending')
                                                    <form action="{{ url('admin/employee/changetimepunchstatus/'.$Employee->id.'/'.$timepunch->id) }}" method="post">
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                                                        <select name="status" class="timepunch_status" onchange="this.form.submit()">
                                                            <option value="">Select Status</option>
                                                            <option value="Approved" @if($timepunch->status == 'Approved') selected @endif >Approved</option>
                                                            <option value="Disapproved" @if($timepunch->status == 'Disapproved') selected @endif >Disapproved</option>
                                                        </select>
                                                    </form>
                                                    @else
                                                    	{{ $timepunch->status }}
	...
	<?php /*?><div id="hours-log-chart-container" style="height: 400px; width: 100%; margin: 0px auto;"></div><?php */?>
