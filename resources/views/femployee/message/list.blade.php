					...
					<div>
						...
						@if(is_array($messages))
							<div class="super-admin-message-right-bottom">
											...
											<div class="super-admin-message-text super-admin-message-text-right">
												...
												<div class="logout-box-main active-msg-drop-down logout-box-main-cargo notnone-6">
													@if( $message['message_read'] == 0 )
														<a href="{{ route("femployee.message_status", ["name" => "read", "id" => $message['message_id'] ])  }}" class="head-active">Mark as Read</a>
													@else
														<a href="{{ route("femployee.message_status", ["name" => "unread", "id" => $message['message_id'] ])  }}">Mark as Unread</a>
													@endif
												</div>
												<div class="clearfix"></div>
												<p>{{ $message['message']  }}</p>
								...
								@if($send_message)
									<form method="POST" action="{{ route('femployee.send_message')  }}">
