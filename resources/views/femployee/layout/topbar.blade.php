  ...
  <div class="head-top">
    ...
    $messages = \App\Models\Messages::where("is_private",1)
    			->where(function ($query) use ($user_id) {
    				$query->where(array('reciever_id'=> $user_id, 'message_to'=> 'Employee'));	
        ...
        if($message_read_by->count() == 0)
        {
          ...
          //{
            ...
            if($type == "Franchise Administration"){
                $url = route("femployee.messages", ["name" => "Franchise_Administration"]);
            }
            ...
            if($type == "Admin Employee"){
                $url = route("femployee.messages", ["name" => "Admin_Employee", "id" => $id]);
            }
            ...
            if($type == "Parent"){
                $url = route("femployee.messages", ["name" => "Parent", "id" => $id]);
            }
    ...
    <div class="head-2">
      ...
      <div class="HeadNotfication dropdown">
        <a href="#" class="dropdown-toggle" id="dropdownMenuButton2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            ...
            @if(Auth::guard('femployee')->user()->profile_picture)
              <img class="cntct-img img-circle" src="{{ asset(Auth::guard('femployee')->user()->profile_picture) }}" />
            @elseif(Auth::guard('femployee')->user()->personal_picture)
              <img class="cntct-img img-circle" src="{{ asset(Auth::guard('femployee')->user()->personal_picture) }}" />
            @elseif(Auth::guard('femployee')->user()->client_profilepicture)
              <img class="cntct-img img-circle" src="{{ asset(Auth::guard('femployee')->user()->client_profilepicture) }}" />    
            @else
        ...
        <div class="logout-box-main notnone-5 dropdown-menu" aria-labelledby="dropdownMenuButton2">
          <a href="#" class="head-active">{{Auth::guard('femployee')->user()->personal_name}}</a>
