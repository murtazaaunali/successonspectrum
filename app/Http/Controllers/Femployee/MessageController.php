...
class MessageController extends Controller
{
    ...
    public function index($name = "", $id = 0)
    {
        ...
        */
		...
		->orWhere(function ($query) use ($user_id){
			$query->where('sender_type','Employee')
	    ...
	    return view('femployee.message.list',$data);
    }
    ...
    public function getMessagingList($type = "Admin BCBA", $name = "", $id = "")
    {
        ...
        if($type == 'Admin Employee'){
    		...
    		$franchise_id = Auth::guard('femployee')->user()->franchise_id;
			$messages = Messages::where(array('message_to'=>'Franchise Administration', 'sender_type'=>'Employee', 'franchise_id'=>$franchise_id))->orderBy('created_at','desc')->first();
        	
        }elseif($type == 'Franchise Employee'){

    		$rcv_id = Auth::guard('femployee')->user()->id;
    		$sender_id = $receiver_id;
			$messages = Messages::where(array('message_to'=>'Employee', 'sender_type'=>'Franchise Employee', 'reciever_id'=> $rcv_id, 'sender_id'=>$sender_id ))->orderBy('created_at','desc')->first();
        ...
        if($type == "Franchise Administration"){
            ...
            $url            = route("femployee.messages", ["name" => "Franchise_Administration", "id" => $id]);
        }

        if($type == "Franchise BCBA"){
            $message_title  = "Franchise BCBA";
            $url            = route("femployee.messages", ["name" => "Franchise_BCBA", "id" => $id]);
        }

        if($type == "Franchise Employee"){
            $message_title  = $name . " <br/> <span class='designation'>Franchise Employee</span>";
            $url            = route("femployee.messages", ["name" => "Franchise_Employee", "id" => $id]);
            $employee = Fuser::find($id);
            if($employee->profile_picture) $message_image = $employee->profile_picture;
        }

        /*if($type == "Employee"){
            $message_title  = $name . " | Employee";
            $url            = route("femployee.messages", ["name" => "Employee", "id" => $id]);
        ...
        if($type == "SOS Distributor"){
            $message_title  = $type;
            $url            = route("femployee.messages", ["name" => "SOS_Distributor"]);
        }
        ...
        if($messages){
            if($receiver_id != 0){
	        	if($type == 'Franchise Employee'){
	        		$emp_id = $receiver_id;
	        		$reciever = Auth::guard('femployee')->user()->id;
	        		$all_messages = Messages::select("id")->where(array('sender_type'=>'Franchise Employee', 'sender_id'=>$emp_id, 'reciever_id'=>$reciever))->get();
				
				}elseif($type == 'Franchise Administration'){
	        		$franchise_id = Auth::guard('femployee')->user()->franchise_id;
	        		$all_messages = Messages::select("id")->where(array('message_to'=>'Franchise Administration', 'franchise_id'=>$franchise_id))->get();
					
				}elseif($type == 'Franchise BCBA'){
	        		$franchise_id = Auth::guard('femployee')->user()->franchise_id;
	        		$all_messages = Messages::select("id")->where(array('message_to'=>'Franchise BCBA', 'franchise_id'=>$franchise_id))->get();
            ...
            if($total_messages > 0){
	            $read_messages    = Messages_read_by::where('user_id',Auth::guard('femployee')->user()->id)->where('user_type','Employee')->whereIn( "message_id", $all_messages->toArray() )->GroupBy("message_id")->get()->count();
    ...
    public function getMessages($name="",$id=0)
    {
        ...
        if($name == "Franchise Administration" || $name == "Franchise BCBA" || $name == "Franchise Employee")
            $franchise_id = Auth::guard('femployee')->user()->franchise_id;
        ...
        if(in_array($name,$allowed_sender))
        {
            ...
            if($name == 'Admin Employee'){
            	$F_employee_id = Auth::guard('femployee')->user()->id;
            	$Admin_employee_id = $id;
            	$all_messages = Messages::where("is_private",1)
	            ->where(function ($query) use ($Admin_employee_id, $F_employee_id) {
	                $query->where(array('reciever_id'=> $F_employee_id, 'franchise_id'=> Auth::guard('femployee')->user()->franchise_id, 'sender_id'=> $Admin_employee_id, 'message_to'=> 'Franchise Employee'));
	            })
	            ...
	            ->where(function ($query) use ($name) {
	                $query->where(array( 'message_to'=> $name, 'franchise_id'=>Auth::guard('femployee')->user()->franchise_id ));
	            })->orderBy("created_at","ASC")->get();
            	...
            	$all_messages = Messages::where("is_private",1)
	            ->where(function ($query) use ($Admin_employee_id, $F_employee_id) {
	                $query->where(array('reciever_id'=> $F_employee_id, 'franchise_id'=> Auth::guard('femployee')->user()->franchise_id, 'sender_id'=> $Admin_employee_id, 'message_to'=> 'Employee', 'sender_type'=> 'Franchise Employee'));
	            })
	            ->orWhere(function ($query) use ($Admin_employee_id, $F_employee_id) {
	            	$query->where(array('reciever_id'=> $Admin_employee_id, 'sender_id'=> $F_employee_id, 'sender_type'=> 'Employee', 'message_to'=> 'Franchise Employee'));
	            })->orderBy("created_at","ASC")->get();
            ...
            if( $all_messages->count() )
            {
                foreach($all_messages as $msg)
                {
                    ...
                    $message_read       = Messages_read_by::where('user_id',Auth::guard('femployee')->user()->id)->where('user_type','Femployee')->where( "message_id", $msg->id )->get()->count();
                    $my_reply           = FALSE;

                    if($msg->sender_id == Auth::guard('femployee')->user()->id && $msg->sender_type == "Employee" ) $my_reply = TRUE;
                    ...
                    if(isset($user->profile_picture) || isset($user->personal_picture)){
                    ...
                    if($msg->reciever_id != Auth::guard('femployee')->user()->id || $msg->sender_type != "Administration")
                        $sender["reply_url"] = route("femployee.messages", ["name" => str_replace(" ","_",$msg->sender_type), "id" => $user->id]);
    ...
    public function send_message(SendMessage $request)
    {
        ...
        if($request->message_to == "Franchise Administration" || $request->message_to == "Franchise BCBA" || $request->message_to == "Franchise Employee")
            $franchise_id = Auth::guard('femployee')->user()->franchise_id;
        ...
        $message_read_by                = new Messages_read_by();//::where('user_id',Auth::guard('femployee')->user()->id)->where('user_type','Admin')->whereIn( "message_id", $all_messages->toArray() )->GroupBy("message_id")->get()->count();
        $message_read_by->message_id    = $new_message->id;
        $message_read_by->user_id       = Auth::guard('femployee')->user()->id;
        $message_read_by->user_type     = 'Employee';
