...
class FranchiseEmployeeBacbExp extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'FR:EmployeeExpBacb';
		...
		if(!$franchises->isEmpty())
		{
			foreach($franchises as $franchise)
			{
				...
				if(!$Employees->isEmpty()){
					...
					{
						if($Employee->bacb_regist_date)
						{
							$regDate = $Employee->bacb_regist_date;
							$regExpDate = date('Y-m-d', strtotime($regDate.'+1 year'));
							$regExpDate = date('Y-m-d', strtotime($regExpDate.'-21 days'));
							
							$currDate = date('Y-m-d');
							
							\Log::info("Areeb Exp:".$regExpDate." Current: ".$currDate);
							if($currDate == $regExpDate){

								$newNoti = new Notifications();
						        $newNoti->title = '<a href="'.url('franchise/employee/view/'.$Employee->id).'">BACB expiration of '.$Employee->fullname.'</a>';
						        $newNoti->description = 'BACB will be expired after 3 weeks';
						        $newNoti->type = 'Notice';
